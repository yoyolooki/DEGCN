...
if __name__ == '__main__':
    ...
    parser.add_argument('--metric', '-m', type=str, choices=['braycurtis', 'canberra', 'chebyshev', 'cityblock',
                                                             ...
                                                             'wminkowski', 'yule'], default='sqeuclidean',
    ...
    parser.add_argument('--mu', '-mu', type=int, default=0.5,
                        help='(0, information) float, Normalization factor to scale similarity kernel when constructing affinity matrix. See Notes of :py:func snf.compute.affinity_matrix for more details. Default: 0.5.')
    ...
    affinity_nets = snf.make_affinity(
        [omics_data_1.iloc[:, 1:].values.astype(float), omics_data_2.iloc[:, 1:].values.astype(float),
         omics_data_3.iloc[:, 1:].values.astype(float)],
        metric=args.metric, K=args.K, mu=args.mu)
    # Final similarity network
    ...
    # fig.savefig('result/SNF_fused_clustermap_0.5.png', dpi=300)
